...
async function handleSettingsGet(req, res, cat) {
  // v4.0 stub: 返回空 schema. 真实 settings 在 v4.0.1 从 daemon/config.js 读
  const schema = {
    connection: {
      cdpPort:        { type: 'number', label: 'CDP 端口', value: 9222 },
      mcpEnabled:     { type: 'boolean', label: '启用 MCP', value: true },
      mcpPort:        { type: 'number', label: 'MCP 端口', value: 9223 },
      httpEnabled:    { type: 'boolean', label: '启用 HTTP API', value: true },
      httpPort:       { type: 'number', label: 'HTTP 端口', value: 9224 },
      requireAuth:    { type: 'boolean', label: 'API token 鉴权', value: false },
    },
    chrome: {
      preferredPath: { type: 'string', label: 'Chrome 路径（优先）', value: '' },
    },
    proxy: {
      mode: { type: 'select', label: '代理模式', value: 'auto', options: [
    ...
    ui: {
      theme: { type: 'select', label: '主题', value: 'dark', options: [
        { value: 'dark', label: '暗色' },
    ...
    logs: {
      level: { type: 'select', label: '日志级别', value: 'info', options: [
      ...
      retentionDays: { type: 'number', label: '保留天数', value: 7 },
    },
    notifications: {
      enabled: { type: 'boolean', label: '启用通知', value: true },
    },
    privacy: {
      redactSensitive: { type: 'boolean', label: '敏感字段自动脱敏', value: true },
    },
    advanced: {
      logLevel: { type: 'select', label: '内部日志级别', value: 'info', options: [
        { value: 'debug', label: 'Debug' },
        { value: 'info', label: 'Info' },
        { value: 'warn', label: 'Warn' },
      ]},
...
async function handleSettingsPost(req, res, cat) {
  let body;
  try { body = await readBody(req); } catch (e) { return jsonResponse(res, 400, { ok: false, error: e.message }); }
  // v4.0 stub: 接受 patch 但真正持久化在 v4.0.1
  jsonResponse(res, 200, { ok: true, cat, patch: body.patch, message: '保存成功 (真实持久化在 v4.0.1)' });
}
